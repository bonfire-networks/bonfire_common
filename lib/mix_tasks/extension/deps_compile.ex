defmodule Mix.Tasks.Bonfire.Extension.Compile do
  use Mix.Task
  import Untangle
  ...
  def run(args) do
    ...
    case OptionParser.parse(args, switches: @switches) do
      ...
      {opts, tail, _} ->
        compile(loaded_by_name(tail, nil, [env: Mix.env()] ++ opts), opts)
    end
  end
  ...
  def loaded_by_name(given, all_deps \\ nil, opts \\ []) do
    ...
    Enum.each(apps, fn app ->
      unless Enum.any?(all_deps, &(&1.app == app)) do
        warn("Unknown dependency #{app} for environment #{opts[:env] || Mix.env()}")
      end
    end)
